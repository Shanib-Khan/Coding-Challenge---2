...
2024-03-25,Latin America,LA002,Accessories,PhoneCase,40,24.99,999.60,No
"""
...
category_region_sales = sales_df.groupby(["Region", "Category"])["Total_Sales"].sum()
top_categories_by_region = category_region_sales.groupby(level=0).idxmax().apply(lambda x: x[1])
...
product_revenue_analysis = product_revenue_analysis.set_index("Product")
...
print("1. Focus more inventory and marketing on the strongest product categories in each region.")
print("2. Review promotion strategy because promoted items should be compared regularly against non-promoted items to confirm they are driving useful revenue.")
print("3. Clean missing data in Units and Unit_Price before future reporting so management decisions are based on complete and accurate information.")